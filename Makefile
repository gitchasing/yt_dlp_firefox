...
install-tor:
	...
	@if ! grep -q "\"--proxy\", \"socks5h://127.0.0.1:9050\"," $(TOR_INSTALL_DIR)/native-messaging-hosts/yt_dlp_firefox; then \
		sed -i "s#\(\"yt-dlp\",\)#\1 \"--proxy\", \"socks5h://127.0.0.1:9050\",#" $(TOR_INSTALL_DIR)/native-messaging-hosts/yt_dlp_firefox ; \
		chmod u+x $(TOR_INSTALL_DIR)/native-messaging-hosts/yt_dlp_firefox ; \
	fi
	@for b in $(APPARMOR_BYPASSES); do \
		if ! grep -q "^  $$b,$$" /etc/apparmor.d/torbrowser.Browser.firefox; then \
			sudo sed -i "s|\(  #include <local/torbrowser.Browser.firefox>\)|  $$b,\n\1|" /etc/apparmor.d/torbrowser.Browser.firefox;  \
		fi; \
